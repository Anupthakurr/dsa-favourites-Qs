...
public:
    int maxArea(vector<int>& height) {
        ...
        // from left: for each i, find farthest right >= height[i]
        for (int i = 0; i < n; i++) {
            for (int j = n-1; j > i; j--) {
                if (height[j] >= height[i]) {
                    ans = max(ans, (j - i) * height[i]);
                    break; // stop at farthest
                }
            }
        }

        // from right: for each i, find farthest left >= height[i]
        for (int i = n-1; i >= 0; i--) {
            for (int j = 0; j < i; j++) {
                if (height[j] >= height[i]) {
